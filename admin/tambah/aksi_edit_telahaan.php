        ...
        if (!in_array($ext, $ekstensi)) {
            ...
            if ($ukuran < 5044070) { //5Mb	
                ...
                if ($b['file_telahaan'] == 'TIDAK ADA FILE') {
                    ...
                    $insert1 = mysqli_query($db, "UPDATE tb_telahaan SET nama_lengkap='$nale', nomor_surat='$nosu', tanggal='$tg', perihal='$per', alamat='$al',  file_telahaan='$xx',keterangan='$ket',  WHERE id_telahaan='$id'");
                    if ($insert1) {
                    ...
                    $xx = $rand . '_' . $filename;
                    move_uploaded_file($_FILES['fileth']['tmp_name'], '../file/telahaan/' . $rand . '_' . $filename);
                    $insert2 =  mysqli_query($db, "UPDATE tb_telahaan SET nama_lengkap='$nale', nomor_surat='$nosu', tanggal='$tg', perihal='$per', alamat='$al', file_telahaan='$xx',keterangan='$ket',  WHERE id_telahaan='$id'");
                    if ($insert2) {
                        $_SESSION['info'] = 'Disimpan';
                        echo "<script>document.location.href='../pengajuan.php'</script>";
                    } else {
