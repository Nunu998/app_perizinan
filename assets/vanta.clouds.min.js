!(function (t, e) {
  ...
  (() => {
      ...
      constructor(t = {}) {
        ...
        try {
          ...
          return (
            ...
            void (
              this.options.backgroundColor && (console.log('[VANTA] Falling back to backgroundColor'), (this.el.style.background = ((s = this.options.backgroundColor), 'number' == typeof s ? '#' + ('00000' + s.toString(16)).slice(-6) : s)))
            )
          );
        }
    ...
    class l extends a {
      ...
      getMaterial() {
        const t = { color: this.options.color, shininess: this.options.shininess, flatShading: !0, side: p.DoubleSide };
      ...
      onInit() {
        ...
        for (t = 0; t <= this.ww; t++)
          for (this.gg[t] = [], e = 0; e <= this.hh; e++) {
            const i = n.length,
              s = new p.Vector3(18 * (t - 0.5 * this.ww), (null == (r = 0) && (r = 0), null == (h = this.waveNoise) && (h = 1), r + Math.random() * (h - r) - 10), 18 * (0.5 * this.hh - e));
        ...
        for (t = 1; t <= this.ww; t++)
          for (e = 1; e <= this.hh; e++) {
            ...
            const n = this.gg[t][e],
              ...
              h = this.gg[t - 1][e],
              p = this.gg[t - 1][e - 1];
            i(0, 1) ? ((s = [p, r, h]), (o = [r, h, n])) : ((s = [p, r, n]), (o = [p, h, n])), a.push(...s, ...o);
          }
        o.setIndex(a), (this.plane = new p.Mesh(o, s)), this.scene.add(this.plane);
      ...
      onUpdate() {
        ...
        for (let t = 0; t < this.plane.geometry.attributes.position.array.length; t += 3) {
          const e = { x: this.plane.geometry.attributes.position.array[t], y: this.plane.geometry.attributes.position.array[t + 1], z: this.plane.geometry.attributes.position.array[t + 2], oy: this.oy[t] };
          if (e.oy) {
            const i = this.options.waveSpeed,
              s = Math.sqrt(i) * Math.cos(-e.x - 0.7 * e.z),
              o = Math.sin(i * this.t * 0.02 - i * e.x * 0.025 + i * e.z * 0.015 + s),
              n = Math.pow(o + 1, 2) / 4;
            (e.y = e.oy + n * this.options.waveHeight), (this.plane.geometry.attributes.position.array[t + 1] = e.y);
          } else this.oy[t] = e.y;
        }
        this.plane.geometry.attributes.position.setUsage(p.DynamicDrawUsage),
          ...
          this.wireframe && (this.wireframe.geometry.fromGeometry(this.plane.geometry), this.wireframe.geometry.computeFaceNormals());
      }
      onMouseMove(t, e) {
        const i = this.camera;
        return (
